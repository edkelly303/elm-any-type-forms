...
{-| Configuration for a custom control. You need to supply the following fields:
  ...
  - `view`: a function that outputs a list of `Html delta` elements based on the `state` of the control. This is similar to an Elm program's view function, except instead of just taking the `state` as an argument, it takes a record containing the `state` plus the id, name, class, and label for the input, and it outputs a list instead of a single element.
